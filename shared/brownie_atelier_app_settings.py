...
container_app__env_vars = [
    ...
    ## mongoDB
    EnvironmentVariable(name='BROWNIE_ATELIER_MONGO__MONGO_SERVER',
                        secure_value=settings.CONTAINER_APP__MONGO_SERVER),
    EnvironmentVariable(name='BROWNIE_ATELIER_MONGO__MONGO_PORT',
                        secure_value=str(settings.CONTAINER_APP__MONGO_PORT)),
    EnvironmentVariable(name='BROWNIE_ATELIER_MONGO__MONGO_USE_DB',
                        value=settings.CONTAINER_APP__MONGO_USE_DB),
    EnvironmentVariable(name='BROWNIE_ATELIER_MONGO__MONGO_USER',
                        secure_value=settings.CONTAINER_APP__MONGO_USER),
    EnvironmentVariable(name='BROWNIE_ATELIER_MONGO__MONGO_PASS',
                        secure_value=settings.CONTAINER_APP__MONGO_PASS),
    EnvironmentVariable(name='BROWNIE_ATELIER_MONGO__MONGO_TLS',
                        value=settings.CONTAINER_APP__MONGO_TLS),
    EnvironmentVariable(name='BROWNIE_ATELIER_MONGO__MONGO_TLS_CA_FILE',
                        secure_value=settings.CONTAINER_APP__MONGO_TLS_CA_FILE),
    EnvironmentVariable(name='BROWNIE_ATELIER_MONGO__MONGO_TLS_CERTTIFICATE_KEY_FILE',
                        secure_value=settings.CONTAINER_APP__MONGO_TLS_CERTTIFICATE_KEY_FILE),
    ## Brownie atelier noticeの設定 (email)
    EnvironmentVariable(name='BROWNIE_ATELIER_NOTICE__SMTP_HOST', secure_value=settings.CONTAINER_APP__NOTICE__SMTP_HOST),
    EnvironmentVariable(name='BROWNIE_ATELIER_NOTICE__SMTP_PORT', secure_value=settings.CONTAINER_APP__NOTICE__SMTP_PORT),
    EnvironmentVariable(name='BROWNIE_ATELIER_NOTICE__FROM_EMAIL', secure_value=settings.CONTAINER_APP__NOTICE__FROM_EMAIL),
    EnvironmentVariable(name='BROWNIE_ATELIER_NOTICE__TO_EMAIL', secure_value=settings.CONTAINER_APP__NOTICE__TO_EMAIL),
    EnvironmentVariable(name='BROWNIE_ATELIER_NOTICE__PASSWORD', secure_value=settings.CONTAINER_APP__NOTICE__PASSWORD),
    EnvironmentVariable(name='BROWNIE_ATELIER_NOTICE__TIMEOUT_LIMIT', secure_value=settings.CONTAINER_APP__NOTICE__TIMEOUT_LIMIT),
    ## scrapy
    ...
    ## prefect2
    EnvironmentVariable(name='PREFECT_HOME',
                        secure_value=settings.CONTAINER_APP__PREFECT_HOME),
    EnvironmentVariable(name='PREFECT__API_URL',
                        secure_value=settings.CONTAINER_APP__PREFECT__API_URL),
    EnvironmentVariable(name='PREFECT__API_KEY',
                        secure_value=settings.CONTAINER_APP__PREFECT__API_KEY),
    EnvironmentVariable(name='PREFECT__WORK_SPACE',
    ...
    EnvironmentVariable(name='PREFECT__DATA',
                        secure_value=settings.CONTAINER_APP__PREFECT__DATA),
    # ## その他
    # EnvironmentVariable(name='CONTAINER_HOME',
    #                     secure_value=f'/home/{settings.CONTAINER_APP__CONTAINER_USER}'),
    ## Azure Storage
    EnvironmentVariable(name='AZURE_STORAGE__BLOB_CONTAINER_NAME',
                        value=storage_settings.AZURE_STORAGE__BLOB_CONTAINER_NAME),
    EnvironmentVariable(name='AZURE_STORAGE__BLOB_FILE_NAME',
...
container_app_auto = Container(
    ...
    # コンテナ起動時に左記のシェルを動かす。
    # command=['bash', '-c', 'while :; do sleep 10000; done'],
    command=['bash', f'/home/{settings.CONTAINER_APP__CONTAINER_USER}/BrownieAtelier/sh/prefect_agent_start.sh'],
    resources=container_app__resource_requirements,
    environment_variables=container_app__env_vars,
    volume_mounts=[container_app__volume_mount_1,
                   container_app__container_mongo__volume_mount]
